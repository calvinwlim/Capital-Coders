...
const fetchStockData = async (ticker, setTradeData) => {
  try {
    const response = await fetch(`http://localhost:3000/getStockPricesFiveDay`, {
      method: "POST",
...
export const StockGraphFiveDay = ({ ticker }) => {
  ...
  return <div id="company-page-stock-graph-container">{tradeData && <Line data={chartDataFiveDay} options={options} plugins={[forceFillPlugin, trackingCirclePlugin]} />}</div>;
};
