...
const fetchStockData = async (ticker, setTradeData) => {
  try {
    const response = await fetch(`http://localhost:3000/getStockPricesFiveYear`, {
      method: "POST",
...
export const StockGraphFiveYear = ({ ticker }) => {
  ...
  return <div id="company-page-stock-graph-container">{tradeData && <Line data={chartDataFiveyear} options={options} plugins={[forceFillPlugin, trackingCirclePlugin]} />}</div>;
};
