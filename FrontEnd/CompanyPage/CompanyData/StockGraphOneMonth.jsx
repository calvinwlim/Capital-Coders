...
const fetchStockData = async (ticker, setTradeData) => {
  try {
    const response = await fetch(`http://localhost:3000/getStockPricesOneMonth`, {
      method: "POST",
...
export const StockGraphOneMonth = ({ ticker }) => {
  ...
  return <div id="company-page-stock-graph-container">{tradeData && <Line data={chartDataOneMonth} options={options} plugins={[forceFillPlugin, trackingCirclePlugin]} />}</div>;
};
...
export const StockGraphOneMonth = ({ ticker }) => {
  ...
  return <div id="company-page-stock-graph-container">{tradeData && <Line data={chartDataOneMonth} options={options} plugins={[forceFillPlugin]} />}</div>;
};
*/
