...
const fetchStockData = async (ticker, setTradeData) => {
  try {
    const response = await fetch(`http://localhost:3000/getStockPricesOneYear`, {
      method: "POST",
...
export const StockGraphOneYear = ({ ticker }) => {
  ...
  return <div id="company-page-stock-graph-container">{tradeData && <Line data={chartDataOneYear} options={options} plugins={[forceFillPlugin, trackingCirclePlugin]} />}</div>;
};
