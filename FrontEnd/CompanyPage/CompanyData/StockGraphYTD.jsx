...
const fetchStockData = async (ticker, setTradeData) => {
  try {
    const response = await fetch(`http://localhost:3000/getStockPricesYTD`, {
      method: "POST",
...
export const StockGraphYTD = ({ ticker }) => {
  ...
  return <div id="company-page-stock-graph-container">{tradeData && <Line data={chartDataYTD} options={options} plugins={[forceFillPlugin, trackingCirclePlugin]} />}</div>;
};
